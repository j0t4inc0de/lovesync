...
- Two stdlib options, same size? Take the one that's correct on edge cases. Being lazy means less code, not a flimsier algorithm.
- When you choose a deliberate simplification (a global lock, a naive heuristic), write a `// ponytail:` comment naming the ceiling of the implementation and the upgrade path. A shortcut is only senior if you know exactly when it breaks.
